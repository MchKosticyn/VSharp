...
type MethodWithBody internal (m : MethodBase) =
    ...
    let hasNonVoidResul = lazy(Reflection.hasNonVoidResult m)
    ...
    let genericArguments = lazy(if isGenericMethod then m.GetGenericArguments() else Array.empty)
    ...
    member x.HasNonVoidResult = hasNonVoidResul.Value
