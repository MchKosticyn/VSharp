...
type private SVMExplorer(explorationOptions: ExplorationOptions, statistics: SVMStatistics, reporter: IReporter) =
    ...
    let solverTimeout =
        ...
        else explorationOptions.timeout / 2 * 1000

    let hasTimeout = explorationOptions.timeout > 0

    let branchReleaseTimeout =
        let doubleTimeout = double explorationOptions.timeout
    ...
    let isTimeoutReached() = hasTimeout && statistics.CurrentExplorationTime.TotalMilliseconds >= explorationOptions.timeout
    ...
    member x.StartExploration (isolated : MethodBase seq) (entryPoints : (MethodBase * string[]) seq) : unit =
        ...
        try
            ...
            let finished =
                ...
                |> fun es -> Task.WaitAll(es, options.timeout)
