...
public static class FuzzerTests
{
    ...
    private static MethodBase[] LoadTestMethods()
    {
        return Assembly
                ...
                .Where(x => x.DeclaringType?.Name.Contains("BoxUnboxWithGeneric") == true)
                // .Where(x => x.Name == "BoxValue")
                .ToArray();
    }
    ...
    private static void RunFuzzer()
    {
        ...
        foreach (var methods in methodsToTest)
        {
            global::System.Console.WriteLine(methods.Name);
        }
