...
// ------------------------------- Types -------------------------------
    ...
    let mostConcreteTypeOfHeapRef state address sightType =
        ...
        if isAssignable locationType sightType || sightType = typeof<Void> then locationType
        else
...
// ------------------------------- Safe reading -------------------------------
    ...
    let readArrayIndex state address indices arrayType =
        ...
        | ConcreteHeapAddress addr, Some concreteIndices when cm.Contains addr ->
             ...
             let obj = cm.ReadArrayIndex addr concreteIndices arrayType (addressType = typeof<string>)
             objToTerm state (fst3 arrayType) obj
...
// ----------------- Unmarshalling: from concrete to symbolic memory -----------------
    ...
    let private unmarshallVector (state : state) address (array : obj[]) typ =
        ...
        let writeIndex state (idx : int) value =
            let value = objToTerm state elemType value
    ...
    let unmarshall (state : state) concreteAddress =
        ...
        let data = cm.Unmarshall concreteAddress typ
        match data with
...
// ------------------------------- Allocation -------------------------------
    ...
    let allocateDelegate state delegateTerm =
        let concreteAddress = freshAddress state
        let typ = typeOf delegateTerm |> ConcreteType
